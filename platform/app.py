...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(dbc.Button("Start Stream", id="start-stream", color="success"), width=3),
        dbc.Col(dbc.Button("Stop Stream", id="stop-stream", color="danger"), width=3),
    ], className="mb-3 text-center"),

    # PTZ Control Buttons
    dbc.Row([
        dbc.Col(width=3),
        dbc.Col(dbc.Button("↑", id="move-up", color="primary", size="lg"), width=2),
        dbc.Col(width=3),
    ], className="mb-2 text-center"),

    dbc.Row([
        dbc.Col(dbc.Button("←", id="move-left", color="primary", size="lg"), width=2, className="text-right"),
        dbc.Col(dbc.Button("STOP", id="stop-move", color="danger", size="lg"), width=2, className="text-center"),
        dbc.Col(dbc.Button("→", id="move-right", color="primary", size="lg"), width=2, className="text-left"),
    ], className="mb-2 text-center"),

    dbc.Row([
        dbc.Col(width=3),
        dbc.Col(dbc.Button("↓", id="move-down", color="primary", size="lg"), width=2),
        dbc.Col(width=3),
    ], className="mb-3 text-center"),
    ...
    # Output message
    html.Div(id="output-message", className="mt-3 text-center", style={"fontWeight": "bold"}),
...
@app.callback(
    ...
    [
        Input("start-stream", "n_clicks"), Input("stop-stream", "n_clicks"),
        Input("move-up", "n_clicks"), Input("move-down", "n_clicks"),
        Input("move-left", "n_clicks"), Input("move-right", "n_clicks"),
        Input("stop-move", "n_clicks"), Input("set-zoom", "n_clicks")
    ],
    [
...
def control_camera(start_stream, stop_stream, up, down, left, right, stop, zoom, camera_id, zoom_level):
    ...
    if button_id == "start-stream":
        ...
        return send_api_request(f"/move/{camera_id}/{direction}" if direction != "Stop" else f"/stop/{camera_id}")
